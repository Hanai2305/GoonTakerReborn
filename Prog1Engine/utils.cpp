...
float GetDistance(float px1, float py1, float px2, float py2)
{
...
{
	float distance = GetDistance(x, y, circle.center.x, circle.center.y);
...
{
	return IsPointInCircle(pos.x, pos.y, circle);
}
...
bool IsPointInRect(const Point2f& pos, const Rectf& rect)
{
	return IsPointInRect(pos.x, pos.y, rect);
}
bool IsOverlapping(const Circlef& circle1, const Circlef& circle2)
{
	float distance = GetDistance(circle1.center, circle2.center);
